...
class ScyllaMachineImageConfigurator(UserData):
    CONF_DEFAULTS = {
        "scylla_yaml": {
            "cluster_name": f"scylladb-cluster-{int(time.time())}",
            "auto_bootstrap": True,
            ...
            "rpc_address": "0.0.0.0",
            "seed_provider": [
                {"class_name": "org.apache.cassandra.locator.SimpleSeedProvider", "parameters": [{"seeds": ""}]}
            ],  # will be configured as a private IP when
    ...
    def configure_cassandra_rackdc(self):
        """Configure cassandra-rackdc.properties for GossipingPropertyFileSnitch."""
        endpoint_snitch = self.scylla_yaml.get("endpoint_snitch")

        if endpoint_snitch != "GossipingPropertyFileSnitch":
            LOGGER.info("Endpoint snitch is %s, skipping cassandra-rackdc.properties generation", endpoint_snitch)
            return

        LOGGER.info("Configuring cassandra-rackdc.properties for GossipingPropertyFileSnitch")

        # Get datacenter and rack from cloud instance
        datacenter = None
        rack = None

        try:
            if hasattr(self.cloud_instance, "region") and self.cloud_instance.region:
                datacenter = self.cloud_instance.region
            if hasattr(self.cloud_instance, "availability_zone") and self.cloud_instance.availability_zone:
                rack = self.cloud_instance.availability_zone
        except Exception as e:
            LOGGER.warning("Failed to get region/availability zone from cloud instance: %s", e)

        if not datacenter or not rack:
            LOGGER.warning("Could not determine datacenter or rack, skipping cassandra-rackdc.properties")
            return

        rackdc_path = Path("/etc/scylla/cassandra-rackdc.properties")
        LOGGER.info("Writing cassandra-rackdc.properties: dc=%s, rack=%s", datacenter, rack)

        try:
            with rackdc_path.open("w") as rackdc_file:
                now = datetime.now(tz=timezone.utc)
                rackdc_file.write(
                    dedent(
                        f"""
                    # Generated by Scylla Machine Image at {now}
                    # Cassandra rack and datacenter configuration for GossipingPropertyFileSnitch
            ...
            LOGGER.info("Successfully created %s", rackdc_path)
        except Exception as e:
            LOGGER.error("Failed to write cassandra-rackdc.properties: %s", e)
    ...
    def set_developer_mode(self):
        ...
        if (
            self.instance_user_data.get("developer_mode", default_developer_mode)
            or self.cloud_instance.is_dev_instance_type()
        ):
    ...
    def configure(self):
        ...
        self.configure_cassandra_rackdc()
