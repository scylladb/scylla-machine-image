...
| `sudo-rs` | Ubuntu 26.04+ ships sudo-rs (Rust rewrite) as the default `sudo` via update-alternatives, alongside classic sudo. sudo-rs uses a closed allow-list sudoers parser and rejects `Defaults logfile=` directive - incompatible with the existing CIS hardening that writes `/etc/sudoers.d/99_logfile`. Purging sudo-rs lets classic sudo (also installed by default) take over, preserving the 24.04 CIS posture. Re-evaluate once ComplianceAsCode/content publishes 26.04 sudo-rs CIS rules. |
