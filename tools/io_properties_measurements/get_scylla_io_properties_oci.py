...
from oci.core.models import LaunchInstanceDetails, CreateVnicDetails, InstanceSourceViaImageDetails, LaunchInstanceShapeConfigDetails
...
def launch_instance(oci_config, image_id, shape, compartment_id, subnet_id, ssh_public_key_path,
                   availability_domain, ocpus=None, memory_gb=None, display_name=None):
    """
...
def get_shapes_in_family(oci_config, compartment_id, availability_domain, family_prefix):
    ...
    try:
        ...
        for shape in shapes_response.data:
            ...
            # Filter shapes that match the family prefix
            if shape_name.startswith(family_prefix):
                # Skip metal shapes if not explicitly requested
                if 'metal' not in shape_name.lower() or 'metal' in family_prefix.lower():
            ...
            if shape.shape == shape_name:
                ...
                result = {
                    'ocpu_options': {
                        ...
                        'max_per_numa_node': int(ocpu_opts.max_per_numa_node) if ocpu_opts and ocpu_opts.max_per_numa_node else None
                    },
                    'memory_options': {
                        'min_in_gbs': float(mem_opts.min_in_g_bs) if mem_opts else 1,
                        'max_in_gbs': float(mem_opts.max_in_g_bs) if mem_opts else 1024,
                        'min_per_ocpu_in_gbs': float(mem_opts.min_per_ocpu_in_gbs) if mem_opts and mem_opts.min_per_ocpu_in_gbs else 1,
                        'max_per_ocpu_in_gbs': float(mem_opts.max_per_ocpu_in_gbs) if mem_opts and mem_opts.max_per_ocpu_in_gbs else 64,
                        'default_per_ocpu_in_gbs': float(mem_opts.default_per_ocpu_in_g_bs) if mem_opts and mem_opts.default_per_ocpu_in_g_bs else 16
                    },
...
def process_instance_parallel(shape, oci_config, image_id, compartment_id, subnet_id, 
                              ssh_public_key_path, ssh_private_key_path, availability_domain,
                              username, ocpus=None, memory_gb=None, override=False):
    """
    ...
    try:
        ...
        instance = launch_instance(
            oci_config, image_id, shape, compartment_id, subnet_id,
            ssh_public_key_path, availability_domain, ocpus, memory_gb, display_name
        )
        ...
        print(f'Error processing {shape_key} (instance {instance_id or "unknown"}) after {elapsed_time:.1f} seconds: {e}')
...
def main():
    ...
    parser.add_argument('--shape', action=SetProvidedAction, help='Instance shape (e.g., VM.Standard.E4.Flex, BM.Standard.E5.192) or family prefix (e.g., VM.Standard.E4)')
    ...
    parser.add_argument('--availability-domain', action=SetProvidedAction, help='Availability domain (e.g., ewbj:US-ASHBURN-AD-1)')
    ...
    parser.add_argument('--ocpus', action=SetProvidedAction,
    ...
    parser.add_argument('--memory-gb', type=int, action=SetProvidedAction, help='Memory in GB for Flex shapes (optional, defaults to default per OCPU for the shape)')
    ...
    parser.add_argument('--parallel', type=int, default=1, help='Number of instances to process in parallel (default: 1)')
    parser.add_argument('--config', default='config_oci.yaml', help='Configuration file to load/save settings (default: config_oci.yaml)')
    parser.add_argument('--save-config', action='store_true', help='Save current settings to config file')
    parser.add_argument('--update-oci-params', action='store_true', help='Update oci_io_params.yaml with collected IO properties')
    parser.add_argument('--oci-params-file', action=SetProvidedAction,
    ...
    if args.list_shapes:
        ...
        if HAS_TABULATE:
            ...
            print(tabulate(flex_rows, headers=['Shape', 'OCPU Range', 'Memory Range (GB)', 'Default Mem/OCPU'], tablefmt='grid'))
        else:
            ...
            for s in flex_shapes:
                ...
                print(f'{s["name"]:<35} {ocpu_range:<15} {mem_range:<20} {s["memory_options"]["default_per_ocpu_in_gbs"]:<15}')
    ...
    if is_family or is_flex_with_multi_ocpus or is_flex_without_ocpus:
            ...
            if shape.endswith('.Flex'):
                ...
                if args.ocpus:
                    ...
                    if shape_options:
                        ...
                        print(f'  OCPU range: {shape_options["ocpu_options"]["min"]} - {shape_options["ocpu_options"]["max"]}')
                        print(f'  Memory per OCPU: {shape_options["memory_options"]["min_per_ocpu_in_gbs"]} - {shape_options["memory_options"]["max_per_ocpu_in_gbs"]} GB')
                        ...
                        if configs and isinstance(configs[0], tuple):
                            ...
                            print(f'  Shape has discrete configurations:')
                        ...
                        print(f'  Could not query shape options, using default configurations')
        ...
        if len(instance_configs) > 1:
            try:
                response = input(f'About to launch {len(instance_configs)} instances. This may incur costs. Continue? (y/N): ')
                if response.lower() != 'y':
        ...
        if args.parallel and args.parallel > 1:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=args.parallel) as executor:
                ...
                for shape, ocpus, mem_gb in instance_configs:
                    future = executor.submit(
                        process_instance_parallel, shape, oci_config, args.image_id,
                        args.compartment_id, args.subnet_id, args.ssh_public_key,
                        args.ssh_private_key, args.availability_domain, args.username,
                        ocpus, mem_gb, args.override
                    )
            ...
            for idx, (shape, ocpus, mem_gb) in enumerate(instance_configs):
                ...
                shape_key, properties = process_instance_parallel(
                    shape, oci_config, args.image_id, args.compartment_id,
                    args.subnet_id, args.ssh_public_key, args.ssh_private_key,
                    args.availability_domain, args.username, ocpus, mem_gb, args.override
                )
                all_properties[shape_key] = properties
        ...
        try:
            if args.dry_run:
                ...
                # Launch instance
                instance = launch_instance(
                    oci_config, args.image_id, args.shape, args.compartment_id,
                    args.subnet_id, args.ssh_public_key, args.availability_domain,
                    ocpus_value, args.memory_gb
                )
                instance_id = instance.id
                ...
                # Get IO properties
                properties = get_io_properties(public_ip, args.ssh_private_key, args.username, args.override, args.shape)
