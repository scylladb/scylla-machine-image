...
def main():
    ...
    parser.add_argument('--key-name', action=SetProvidedAction, help='Name of existing EC2 key pair in your AWS account')
    parser.add_argument('--key-path', action=SetProvidedAction, help='Path to private key file')
    parser.add_argument('--security-group', action=SetProvidedAction, help='Security group name or ID that allows SSH access (port 22)')
    ...
    if not args.security_group and not args.dry_run:
        parser.error('--security-group is required. Specify a security group name or ID that allows SSH access (port 22).')
