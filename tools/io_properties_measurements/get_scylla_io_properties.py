...
import stat
import re
# from update_aws_io_params import update_aws_io_params
try:
...
def get_wait_times(instance_type):
    ...
    if '.' in instance_type:
        size_part = instance_type.split('.')[-1]
    else:
        size_part = 'large'  # default
    ...
    # Use same value for all instance types, ignoring multiplier
    initial_wait = base_initial_wait
    io_wait_minutes = base_io_wait
    
    return initial_wait, io_wait_minutes

def launch_instance(ami_id, instance_type, key_name='my-key-pair', security_group='sg-088128b2712c264d1', subnet_id=None, region='us-east-1'):
    """
    ...
    if not subnet_id:
        ...
        az_response = ec2_client.describe_instance_type_offerings(
            ...
            Filters=[
                ...
                {'Name': 'location', 'Values': ['us-east-1*']}  # All AZs in us-east-1
            ]
        )
        ...
        if subnet_response['Subnets']:
            ...
            raise Exception(f'No subnets found in VPC {vpc_id} for security group {security_group} in supported availability zones')
...
def get_io_properties(instance, key_path, username='scyllaadm', override=False):
    ...
    if os.name != 'nt':  # Not Windows
        ...
        if key_permissions & stat.S_IRWXG or key_permissions & stat.S_IRWXO:
            print(f"Warning: SSH key file has group/other permissions. Fixing permissions to 0600.")
    ...
    for attempt in range(5):
        try:
            print(f'Attempting SSH connection (attempt {attempt + 1}/5) to {instance.public_ip_address} as {username}...')
            ssh.connect(
    ...
    try:
        for i in range(max_attempts):
            ...
            raise Exception(f'IO properties file not found after {io_wait_minutes} minutes for {instance.instance_type}')
...
def get_instance_types_in_family(family_prefix, ec2_client):
    ...
    try:
        ...
        size_order = ['large', 'xlarge', '2xlarge', '4xlarge', '8xlarge', '12xlarge', '16xlarge', '24xlarge', '32xlarge', '48xlarge']
...
def launch_instance_for_family(ami_id, instance_type, key_name, security_group, subnet_id, ec2_client):
    ...
    """
    ec2 = boto3.resource('ec2', region_name=ec2_client._client_config.region_name)
...
def format_output(properties, format_type='yaml', instance_type=''):
    """
    Format IO properties output in different formats
    """
    if format_type == 'json':
...
def process_instance_parallel(instance_type, ami_id, key_name, security_group, subnet_id, key_path, username, region, override=False):
    """
    ...
    try:
        ...
        instance = launch_instance_for_family(
            ...
            security_group, subnet_id, ec2_client
        )
...
def sort_aws_instance_types(instance_types):
    ...
    size_order = [
        'medium', 'large', 'xlarge', '2xlarge', '3xlarge', '4xlarge', '6xlarge', 
        '8xlarge', '9xlarge', '12xlarge', '16xlarge', '18xlarge', '24xlarge', 
        '32xlarge', '48xlarge', 'metal'
    ]
    ...
    def parse_family_for_sorting(family):
        ...
        # Match pattern: letters + optional number + optional letters
    ...
    for family, instances in updates_by_family.items():
        ...
        for instance_type in sorted_instances:
            try:
                ...
                print(f"Would update parameters for {instance_type} in {params_file}")
                update_count += 1
...
def sort_instances_by_size(instance_types):
    ...
    """
    size_order = ['medium', 'large', 'xlarge', '2xlarge', '3xlarge', '4xlarge', 
                  '6xlarge', '8xlarge', '9xlarge', '12xlarge', '16xlarge', 
                  '18xlarge', '24xlarge', '32xlarge', '48xlarge', 'metal', 'ALL']
...
def update_single_instance_params(instance_type, properties, params_file_path):
    ...
    try:
        # from update_aws_io_params import update_aws_io_params
        # return update_aws_io_params(properties, instance_type, params_file)
        print(f"Would update parameters for {instance_type} in {params_file}")
...
def main():
    ...
    parser.add_argument('--instance-type', action=SetProvidedAction, help='Instance type (e.g., i8g.2xlarge) or family (e.g., i8g) - hyphens will be converted to dots')
    parser.add_argument('--key-name', action=SetProvidedAction, help='Key pair name')
    parser.add_argument('--key-path', action=SetProvidedAction, help='Path to private key file')
    parser.add_argument('--security-group', action=SetProvidedAction, help='Security group name or ID')
    parser.add_argument('--subnet-id', action=SetProvidedAction, help='Subnet ID (optional, auto-detected from security group VPC if not specified)')
    parser.add_argument('--username', action=SetProvidedAction, default='scyllaadm', help='SSH username (default: scyllaadm for ScyllaDB)')
    parser.add_argument('--dry-run', action='store_true', help='Show what would be done without launching instances')
    parser.add_argument('--max-instances', action=SetProvidedAction, type=int, help='Maximum number of instances to process for a family (default: no limit)')
    parser.add_argument('--output-format', action=SetProvidedAction, choices=['yaml', 'json', 'table'], default='yaml', help='Output format for IO properties (default: yaml)')
    parser.add_argument('--parallel', action=SetProvidedAction, type=int, default=1, help='Number of instances to process in parallel (default: 1)')
    parser.add_argument('--region', action=SetProvidedAction, default='us-east-1', help='AWS region to use (default: us-east-1)')
    parser.add_argument('--config', default='config.yaml', help='Configuration file to load/save settings (default: config.yaml)')
    parser.add_argument('--save-config', action='store_true', help='Save current settings to config file')
    parser.add_argument('--update-aws-params', action='store_true', help='Update aws_io_params.yaml with collected IO properties')
    parser.add_argument('--aws-params-file', action=SetProvidedAction, 
    ...
    provided_args.update(['dry_run', 'save_config', 'override'])  # These are always boolean flags
    ...
    args.key_name = config.get('key_name', 'my-key-pair')
    args.key_path = config.get('key_path')
    args.security_group = config.get('security_group', 'sg-088128b2712c264d1')
    ...
    args.output_format = config.get('output_format', 'yaml')
    ...
    if '.' not in args.instance_type:
        ...
        if len(instance_types) > 1:
            try:
                response = input(f'About to launch {len(instance_types)} instances. This may incur costs. Continue? (y/N): ')
                if response.lower() != 'y':
        ...
        if args.parallel and args.parallel > 1:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=args.parallel) as executor:
                futures = {executor.submit(process_instance_parallel, it, args.ami, args.key_name, 
                                         args.security_group, args.subnet_id, args.key_path, 
                                         args.username, args.region, args.override): it for it in instance_types}
            ...
            for idx, instance_type in enumerate(instance_types):
                ...
                try:
                    ...
                    instance = launch_instance_for_family(
                        args.ami, instance_type, args.key_name, 
                        args.security_group, args.subnet_id, ec2_client
                    )
        ...
        for instance_type, props in all_properties.items():
            ...
            if 'error' in props:
                ...
                print(format_output(props, args.output_format, instance_type))
        ...
        try:
            if args.dry_run:
                ...
                try:
                    ...
                    instance = launch_instance(args.ami, args.instance_type, args.key_name, args.security_group, args.subnet_id, args.region)
                    instance_id = instance.id
                    ...
                    print(format_output(properties, args.output_format, args.instance_type))
            ...
            if 'groupId' in error_message and 'invalid' in error_message.lower():
                ...
                print('You can list available security groups with: aws ec2 describe-security-groups --query "SecurityGroups[*].{Name:GroupName,ID:GroupId}"')
