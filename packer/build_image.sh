...
run_post_processor() {
    if [ "$TARGET" = "oci" ]; then
        ...
        IMAGE_OCID=$(grep 'A disk image was created' "$PACKER_LOG_PATH" | grep -o 'ocid1.image.oc1.[^ ]*' | tail -n 1)
...
if [ "$TARGET" = "aws" ]; then
    ...
    if [ -n "$OCI_BASE_IMAGE_OCID" ]; then
        PACKER_ARGS+=(-var base_image_ocid="$OCI_BASE_IMAGE_OCID")
    fi
fi
...
if $DRY_RUN ; then
  ...
  case "$TARGET" in
    ...
    "azure")
      ...
      ;;
    "oci")
      grep "oracle-oci:" $PACKER_LOG_PATH
      GREP_STATUS=$?
      ;;
    *)
