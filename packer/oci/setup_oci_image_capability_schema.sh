...
if [[ -z "$COMPARTMENT_ID" ]]; then
    ...
    IMAGE_DETAILS=$(oci compute image get --image-id "${IMAGE_ID}" 2>/dev/null || true)
...
GLOBAL_SCHEMA=$(oci compute global-image-capability-schema list --all 2>/dev/null | jq -r '.data[0]')
...
# Check if the image already has a schema attached
print_info "Checking if image already has a capability schema..."
IMAGE_SCHEMA_ID=$(oci compute image get --image-id "${IMAGE_ID}" 2>/dev/null | jq -r '.data."image-capability-schema-id" // empty' || true)

SCHEMA_ID=""

if [[ -n "$IMAGE_SCHEMA_ID" ]]; then
    print_warning "Image already has a capability schema attached: ${IMAGE_SCHEMA_ID}"
    
    # Get the schema details
    EXISTING_SCHEMA_DETAILS=$(oci compute image-capability-schema get --compute-image-capability-schema-id "${IMAGE_SCHEMA_ID}" 2>/dev/null || true)
    EXISTING_SCHEMA_NAME=$(echo "$EXISTING_SCHEMA_DETAILS" | jq -r '.data."display-name" // empty')
    
    print_info "Existing schema name: ${EXISTING_SCHEMA_NAME}"
    read -p "Do you want to update the existing schema with new capabilities? (y/n): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        print_info "Updating existing Image Capability Schema..."
        
        UPDATE_OUTPUT=$(oci compute image-capability-schema update \
            --compute-image-capability-schema-id "${IMAGE_SCHEMA_ID}" \
        ...
        SCHEMA_ID="$IMAGE_SCHEMA_ID"
        print_info "Using existing schema without changes: ${SCHEMA_ID}"
    fi
else
    # Check if schema with the same name already exists in compartment
    print_info "Checking if schema with name '${SCHEMA_NAME}' already exists..."
    EXISTING_SCHEMA=$(oci compute image-capability-schema list \
        --compartment-id "${COMPARTMENT_ID}" \
        --display-name "${SCHEMA_NAME}" \
        --all 2>/dev/null | jq -r '.data[0].id // empty' || true)

    if [[ -n "$EXISTING_SCHEMA" ]]; then
        print_warning "Schema '${SCHEMA_NAME}' already exists: ${EXISTING_SCHEMA}"
        read -p "Do you want to use the existing schema? (y/n): " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            SCHEMA_ID="$EXISTING_SCHEMA"
            print_success "Using existing schema: ${SCHEMA_ID}"
        else
            print_error "Operation cancelled by user"
            exit 1
        fi
    else
        print_info "Creating new Image Capability Schema..."
...
VERIFY_OUTPUT=$(oci compute image get \
    --image-id "${IMAGE_ID}" \
    2>/dev/null | jq -r '.data."image-capability-schema-id" // empty' || true)
...
echo "  oci compute image-capability-schema get --compute-image-capability-schema-id ${SCHEMA_ID}"
echo ""
